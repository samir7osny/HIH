...
$(document).ready(function () {
    ...
    $(".popUpWindow .search").on("click", ".member:not(.addMember)",function (e) {
        e.preventDefault();
        if($('.modirators .members .flexBox .member[m="' + $(this).attr('m') + '"]').length == 0){
            var newEle = $(this).clone().append('<span class="removeButton"><i class="fa fa-minus-square" aria-hidden="true"></i></span><input name="modirator[]" hidden type="integer" value="' + $(this).attr('m') + '">');
            $('.modirators .members .flexBox').append(newEle);
        }
        if(SearchOpen==true){
            SearchOpen=false;
            $(".popUpWindow .search").removeClass('on');
    ...
    $(".modirators").on("click", ".member .removeButton",function (e) {
        e.preventDefault();
        e.stopPropagation();
